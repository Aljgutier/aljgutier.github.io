...
Though in the diagram the phases appear to flow from start to finish, there are usually many iterations within and between the stages as insights and data understanding is developed. Also, project often focus on a subset of the life-cycle. For example, a discovery project may focus primarily in the data discovery, and insights generation. After initial data discovery, then BI insights and dashboards can be created. Additionally, a prototype ML model can go into the ML Operations phase moving from prototype to full-scale production with real (possibly real-time) customer data along with management of the model (ML and operations performance) in production.
