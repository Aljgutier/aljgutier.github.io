...
While developing machine learning models, there is a set of best practices to deliver consistent and effective results. Of course, one essential best practice is to ensure there is a sufficiently predictive data set, without which it is not possible to develop an effective model. While developing ML/AI models and managing them through their life cycle, it is helpful to have a set of best practices inclusive of the entire machine learning model life-cycle. However, It seems there is no one definitive reference, that describes this data science modeling process. Several authors have published articles and books outlining methods for developing machine learning models. A few good sources are listed in the references. $^{1,2,3}$  Informed by these best practices, a data science modeling process is depicted in the figure above. This picture represents the machine learning modeling process representing  the life-cycle of a model.
...
To achieve consistent and effective results, developing a machine learning model is best done within a data science methodology. It is worth noting that sometimes the data science process's objective is to generate business insights and not necessarily the development of an ML model.  However, the scope of this exercise is for creating, deploying, and managing machine learning models, hence a more extensive process. Though in the diagram the phases appear to flow from start to finish, there are usually many iterations within and between the stages as insights are developed and understanding of which data transformations and models work best. The diagram is a template. For example, depending on the specifics, some steps can be merged, simplified.
